...
export const TAGS_FILENAME = "TAGS.md"; // changes
...
export async function ensureAgentWorkspace(params?: {
  ...
  const tagsPath = path.join(dir, TAGS_FILENAME); // changes
  ...
  const tagsTemplate = await loadTemplate(TAGS_FILENAME); //changes
  ...
  await writeFileIfMissing(tagsPath, tagsTemplate); // changes
...
export async function loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
  ...
  const entries: Array<{
    ...
    {
      name: TAGS_FILENAME,
      filePath: path.join(resolvedDir, TAGS_FILENAME),
    },
  ];
